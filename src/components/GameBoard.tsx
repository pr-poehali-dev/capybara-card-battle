...
const GameBoard: React.FC = () => {
  ...
  }, [currentTurn, gameStatus, capibaraPlayCard, playedCards.player, evaluateRound]);
  ...
  return (
    <div className="flex flex-col h-full min-h-[calc(100vh-2rem)] p-4">
      ...
      <div className="text-center mb-6">
        <h1 className="text-3xl font-bold mb-2 font-rubik">Магическая Колода: Битва с Капибарой</h1>
        <div className="flex justify-center space-x-8">
          <div className="text-lg">
            <span className="font-semibold">Ваши очки:</span> {playerScore}
          </div>
          <div className="text-lg">
            <span className="font-semibold">Очки капибары:</span> {capibaraScore}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-between">
        ...
        <div className="flex flex-col items-center mb-4">
          <CapibaraOpponent 
            ...
            expression={
              gameStatus === 'capibaraWon' ? 'happy' : 
              gameStatus === 'playerWon' ? 'sad' : 'neutral'
            }
          />
          ...
          <div className="flex justify-center mt-2">
            {capibaraHand.map((_, index) => (
              <div 
                ...
                style={{ backgroundImage: "url('data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"40\" height=\"40\" viewBox=\"0 0 40 40\"><circle cx=\"20\" cy=\"20\" r=\"18\" fill=\"none\" stroke=\"%23A67C52\" stroke-width=\"2\"/><text x=\"20\" y=\"25\" font-family=\"Arial\" font-size=\"10\" text-anchor=\"middle\" fill=\"%23A67C52\">К</text></svg>')" }}
              />
            ))}
          </div>
        </div>
          ...
          {gameStatus === 'waiting' ? (
            ...
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">
                {gameStatus === 'playerWon' ? 'Поздравляем! Вы победили!' : 
                 gameStatus === 'capibaraWon' ? 'Капибара победила! Попробуйте еще раз.' : 
                 'Ничья! Равная борьба.'}
              </h2>
              <Button onClick={resetGame} className="mt-4 bg-secondary hover:bg-secondary/80">
                Сыграть еще раз
              </Button>
            </div>
          )}
        ...
        <div className="mt-4">
          ...
          {gameStatus === 'playing' && selectedCard && currentTurn === 'player' && (
            <div className="flex justify-center">
              <Button 
